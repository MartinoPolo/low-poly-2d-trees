...
describe('REQ-T: Trunk & Branch Generation', () => {
	...
	describe('REQ-T-02b: canopy follows trunk height', () => {
		...
		it('trunk top enters canopy by ≥10px (sampled) across oak, pine, birch at 50/100/150', () => {
			// The analytical clamp enforces 15 px (TRUNK_ENTRY_MIN_PX), but
			// sampled triangle vertices have radial jitter. 10px threshold
			// accounts for this while still proving trunk penetration.
			for (const shape of ['oak', 'pine', 'birch'] as const) {
				for (const trunkHeight of [50, 100, 150]) {
					...
					expect(geo.anchors.trunkTop.y + 10).toBeLessThanOrEqual(canopyMaxY);
				}
			}
		});
	});
...
describe('B1: fir uses tier-based rendering', () => {
	...
	it('fir with branchCount=0 produces zero branches', () => {
		const geo = generateTree(
			makeConfig({ shape: 'fir', seed: 42, branchCount: 0, blobCount: 4 }),
		);
		expect(geo.branchTriangles.length).toBe(0);
